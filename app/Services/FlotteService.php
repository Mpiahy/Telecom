...
class FlotteService
{
    ...
    public function formatSuiviFlotteData($affectations, int $annee): array
    {
        $rows = [];
        ...
        foreach ($affectations as $affectation) {
            ...
            // 🔹 Ajout des coûts des opérations après prorata sur le mois suivant
            $operations = DB::table('view_historique_operation')
                ->where('id_ligne', $affectation->id_ligne)
            ...
            foreach ($operations as $operation) {
                ...
                $row["mois_$moisFacturation"] += round($operation->prix_ht_remise_prorata, 2);
                $totalAnnuel += round($operation->prix_ht_remise_prorata, 2);
            }
